...
export async function createReviewInvitation(formData: FormData) {
  ...
  if (orderId) {
    ...
    if (error || !order) throw new Error("Selected order could not be found.");
  ...
  const { error } = await admin.from("review_invitations").insert({
    order_id: orderId,
...
export async function revokeReviewInvitation(formData: FormData) {
  ...
  if (error) throw new Error("Unable to revoke invitation.");
  revalidatePath("/admin/reviews");
}
...
export async function updateReviewStatus(formData: FormData) {
  ...
  if (!id || !REVIEW_STATUSES.has(status)) throw new Error("Invalid review update.");
  ...
  if (error) throw new Error("Unable to update review.");
  revalidatePath("/admin/reviews");
  revalidatePath("/");
}
...
export async function toggleFeaturedReview(formData: FormData) {
  ...
  if (error) throw new Error("Unable to update featured status.");
  revalidatePath("/admin/reviews");
  revalidatePath("/");
}
...
export async function deleteReview(formData: FormData) {
  ...
  await admin
    .from("review_invitations")
    .update({ review_id: null, updated_at: new Date().toISOString() })
    .eq("review_id", id);

  const { error } = await admin.from("reviews").delete().eq("id", id);
  if (error) throw new Error("Unable to delete review.");
